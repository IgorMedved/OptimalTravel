...
public class App {
	...
	public static void main(String[] args) {
		LineMapCreator processor = new LineMapCreator(); // lineMapCreator processor processes the information supplied by file reader and creates LineNetwork as a Map
		LineInfoFileReader reader = new LineInfoFileReader(processor); // initializes file reader with the processor
		reader.readLineInfoFile(); // reads information from user supplied file and passes it to processor for processing 
		LineNetwork lineMap = processor.getNetwork(); // receive network of lines created in a previous step
		lineMap.printLines();
		NextStationMap nextStations = new NextStationMap();
		nextStations.createNextStationMap(lineMap);
		nextStations.printNextStations();
		RouteFinder routeFinder = new RouteFinder(nextStations, "Mighty Gumball, Inc", "Choc-O-Holic, Inc.");
		routeFinder.findRoute(nextStations);
		routeFinder.printRoute(lineMap);
	}
